...
fn echange_count_1(){
    ...
    let mut dlr_fix_card_vec:Vec<Card> = vec![];
    dlr_fix_card_vec.push(Card::new(Rank::Two,Suit::Hearts));
    // dlr_fix_card_vec.push(Card::new(Rank::King,Suit::Clubs));
    // fix_card_vec.push(Card::new(Rank::Queen,Suit::Clubs));
    let dlr_flat_set = FlatSet::new_from_card_vec(&dlr_fix_card_vec);
    let dlr_fix_cards = FixMix::new_from_flat(dlr_flat_set.flat_bin);
    // let fix_cards = FixMix::new_from_flat(0);

    println!("dlr_fix_cards {}",dlr_fix_cards);


    let mut dlr_comb_vec:Vec<CardDig16> = vec![];
    dlr_comb_vec.push(rank_number_to_card16(Rank::Ten.rank_number()));
    dlr_comb_vec.push(rank_number_to_card16(Rank::Five.rank_number()));
    dlr_comb_vec.push(rank_number_to_card16(Rank::Four.rank_number()));
    dlr_comb_vec.push(rank_number_to_card16(Rank::Two.rank_number()));
    dlr_comb_vec.push(rank_number_to_card16(Rank::Three.rank_number()));

    let dlr_pyr_comb5 = PyrSet::new_from_vec(&comb_vec, false);
    println!("dlr_pyr_comb5 {}",dlr_pyr_comb5);

    let deck_info= DeckInfo{
        deck_before:deck_mix,
        pyr_after: from_flat_64_to_pyr_64_wo_flush(deck_mix.pyr & !virt_cards.pyr ),
        virt_cards:virt_cards,
    ...
    let (comb5_unfl_amount,comb5_fl_info, virt_cards)  = dlr_comb_count(&dlr_fix_cards,&deck_info,dlr_pyr_comb5.pyr_bin);
